        ...
        with open(task_file, 'r') as f:
            try:
                ...
                if isinstance(data, list):
                    logger.warning(f"⚠️ Schema mismatch: Task file {task_file} is a list. Extracting first element.")
...
def load_pipeline_manifest(repo_path: Path, pipeline_id: str) -> dict:
    ...
    # Defensively normalize array wrappers if generated by legacy systems or test mocks
    if isinstance(data, list):
        logger.warning(f"⚠️ Schema contract violation: Manifest {manifest_matches[0]} is an array, expected an object. Coercing to first item.")
        data = data[0] if data else {}
