...
def test_main_exits_when_inputs_missing(
    ...
    """
    Targets: Ensures execution halts immediately upon first sys.exit(1) call.
    """
    # 1. Setup
    ...
    # 2. Configure Mock to Halt Execution
    mock_fetch.side_effect = FileNotFoundError("CRITICAL: Input missing")
    mock_exit.side_effect = SystemExit  # This makes the test stop on the first exit

    # 3. Execution (Assert it raises SystemExit)
